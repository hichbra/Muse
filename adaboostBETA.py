...
def main():
    ...
    my_data = genfromtxt("/home/hicham/Bureau/Stage/Dataset/dataset_1/datasetAssisMD.csv", delimiter=';')
    input_signal_l_ear = np.reshape(my_data[:,18:19].T, -1, 2) # 2eme colonne
    ...
    input_signal_l_forehead = np.reshape(my_data[:,19:20].T, -1, 2) # 2eme colonne
    ...
    input_signal_r_forehead = np.reshape(my_data[:,20:21].T, -1, 2) # 2eme colonne
    ...
    input_signal_r_ear = np.reshape(my_data[:,21:22].T, -1, 2) # 2eme colonne
