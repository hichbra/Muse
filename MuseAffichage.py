...
def updateInterface(graph, dataList, output_signal_l_ear):
    graph.clf()

    #graph = graph.add_subplot(111)

    graph.plot(output_signal_l_ear)
    graph.plot(dataList)
    #for i in range(len(dataList)):
    #    plt.scatter(i, dataList[i])

    graph.grid(True)
    graph.pause(0.05)

    #graph.legend(bbox_to_anchor=(0.88, 0.88), loc=2, borderaxespad=0.)
    graph.draw()
...
def main():
    ...
    while 1:
    	...
    	try:
            ...
            nombreAleatoire = randrange(-50,50)

            TAILLE_FENETRE = 71
            if i < TAILLE_FENETRE:
                fenetre_a_predire.append(nombreAleatoire)
                output_signal_l_ear = signal.filtfilt(b, a, fenetre_a_predire)

                #print(len(output_signal_l_ear))
            ...
            #    fenetre_a_predire = np.array(fenetre_a_predire)
                ...
                print(output_signal_l_ear)
                #print(len(output_signal_l_ear))

                fenetre_a_predire = np.delete(fenetre_a_predire, 0) # Supprime le premier enregistrement
                fenetre_a_predire = np.append(fenetre_a_predire, nombreAleatoire) # Ajoute un nouvel enregistrement



            i=i+1
            updateInterface(plt, fenetre_a_predire, output_signal_l_ear)
