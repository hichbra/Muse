...
def main():
    ...
    input_signal_Random = np.reshape(my_data[:,18:19].T, -1, 2) # 2eme colonne
    ...
    plt.plot(input_signal_Random, label='Signal beta_l_ear')
    plt.plot(output_signal_Random, label='Signal filtre beta_l_ear')

    prediction_visualisation = [ECHELLE_PREDICTION if x==1 else x for x in prediction_visualisation]
    plt.plot(prediction_visualisation, label='Prediction beta_l_ear')
    ...
    sess = tf.Session()
    new_saver2 = tf.train.import_meta_graph('Graph/EEG/BETA/l_forehead/l_forehead.meta')
    new_saver2.restore(sess, tf.train.latest_checkpoint('./Graph/EEG/l_forehead'))

    input_signal_Random2 = np.reshape(my_data[:,19:20].T, -1, 2) # 2eme colonne
    ...
    #plt.plot(input_signal_Random2, label='Signal beta_l_forehead')
    #plt.plot(output_signal_Random2, label='Signal filtre beta_l_forehead')

    prediction_visualisation = [ECHELLE_PREDICTION if x==1 else x for x in prediction_visualisation]
    plt.plot(prediction_visualisation, label='Prediction beta_l_forehead')
    print("TAILLE",prediction_visualisation)

    """ -------------------------- SIGNAL 3 r_forehead -------------------------- """

    sess = tf.Session()
    new_saver3 = tf.train.import_meta_graph('Graph/EEG/BETA/r_forehead/r_forehead.meta')
    new_saver3.restore(sess, tf.train.latest_checkpoint('./Graph/EEG/r_forehead'))

    input_signal_Random3 = np.reshape(my_data[:,20:21].T, -1, 2) # 2eme colonne
    ...
    #plt.plot(input_signal_Random3, label='Signal beta_r_forehead')
    #plt.plot(output_signal_Random3, label='Signal filtre beta_r_forehead')

    prediction_visualisation = [ECHELLE_PREDICTION if x==1 else x for x in prediction_visualisation]
    plt.plot(prediction_visualisation, label='Prediction beta_r_forehead')
    print("TAILLE",prediction_visualisation)

    """ -------------------------- SIGNAL 4 r_ear -------------------------- """

    sess = tf.Session()
    new_saver4 = tf.train.import_meta_graph('Graph/EEG/BETA/r_ear/r_ear.meta')
    new_saver4.restore(sess, tf.train.latest_checkpoint('./Graph/EEG/r_ear'))

    input_signal_Random4 = np.reshape(my_data[:,21:22].T, -1, 2) # 2eme colonne
    ...
    #plt.plot(input_signal_Random4, label='Signal beta_r_ear')
    #plt.plot(output_signal_Random4, label='Signal filtre beta_r_ear')

    prediction_visualisation = [ECHELLE_PREDICTION if x==1 else x for x in prediction_visualisation]
    plt.plot(prediction_visualisation, label='Prediction beta_r_ear')
    print("TAILLE",prediction_visualisation)
